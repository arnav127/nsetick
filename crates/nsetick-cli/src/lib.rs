//! The `nsetick` command line, as a library.
//!
//! The standalone binary (`src/main.rs`) and the Python package's `nsetick` command both call
//! [`run`], so `pip install nsetick` gives the same command as the release archives.
...
/// Run the command line with these arguments (the first is the program name) and return the
/// process exit code. Errors are printed here, as a `main` returning `Result` would.
