...
fn main() -> Result<()> {
    ...
    let input = args.next().context("usage: dump_fills <orders.parquet> <fills.csv>")?;
    let output = args.next().context("usage: dump_fills <orders.parquet> <fills.csv>")?;
    ...
    writeln!(out, "symbol,timestamp,price,quantity,incoming_order,resting_order,from_hidden")?;
    ...
    let mut books: std::collections::HashMap<String, (OrderBook, VecDeque<OrderEvent>)> =
        Default::default();
    let emit = |sym: &str, book: &mut OrderBook, ev: &OrderEvent, out: &mut BufWriter<File>| -> Result<()> {
        book.apply(ev);
    ...
    for batch in reader {
        for (sym, events) in events_by_symbol(&batch?)? {
            ...
            for ev in &events {
                ...
                while pending.front().is_some_and(|f| f.timestamp + window < ev.timestamp) {
