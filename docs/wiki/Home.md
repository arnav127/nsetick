...
| **Verify** | Every byte offset lives in one versioned spec, checked against each file's record length; the replay reproduces NSE's own trade records exactly, order for order |
